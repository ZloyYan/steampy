...
class LoginExecutor:
    ...
    def _update_steam_guard(self, login_response: Response) -> None:
        ...
        try:
            ...
            with client_id_file.open('w') as file:
                json.dump({'client_id': client_id, 'steamid': steamid, 'request_id': request_id}, file)

        except KeyError:
            # Чтение данных из файла, если ошибка KeyError
            if client_id_file.exists():
                with client_id_file.open() as file:
                    data = json.load(file)
                client_id = data['client_id']
                steamid = data['steamid']
                request_id = data['request_id']
            else:
                raise Exception('Cannot retrieve Steam guard data')
    ...
    def _pool_sessions_steam(self, client_id: str, request_id: str) -> None:
        ...
        try:
            ...
            try:
                ...
                # Если файла нет, присваиваем пустую строку или другое подходящее значение
                self.refresh_token = ''
